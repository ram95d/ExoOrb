...
if "last_input_method" not in st.session_state:
    st.session_state.last_input_method = None # Will be set after the radio button
...
# --- Input Method Change Handling ---
if st.session_state.last_input_method != input_method:
    st.session_state.data_df = None
    st.session_state.initial_maximize_selection_guess = []
    # Reset other relevant session states if they were specific to the old method
    # For instance, if form values were stored in session_state, clear them here.
    # However, method_names and manual_factor_names are rebuilt from widgets, which is fine.
    st.session_state.last_input_method = input_method
    # st.experimental_rerun() # Consider if UI doesn't update smoothly, but usually not needed

# These will store the factor/method names defined during manual entry for the current run
manual_factor_names = []
method_names = []
...
if input_method == "Upload Excel File":
    ...
    if uploaded_file is not None:
        try:
            ...
            st.session_state.data_df = loaded_df # Store in session state
            ...
            st.session_state.data_df = None # Ensure data_df is None on error
            st.session_state.initial_maximize_selection_guess = []
    else:
        # If file is deselected or never selected while in "Upload Excel File" mode
        st.session_state.data_df = None
        st.session_state.initial_maximize_selection_guess = []
    ...
    temp_method_names = [] # Use temporary list to build method_names
    ...
    method_names = temp_method_names # Assign to the main variable for this run
    
    st.sidebar.subheader("Parameter Names")
    temp_manual_factor_names = [] # Use temporary list
    ...
    manual_factor_names = temp_manual_factor_names # Assign to the main variable for this run
    ...
    if method_names and manual_factor_names:
        ...
        if submitted:
            if method_data:
                ...
                st.session_state.data_df = local_df # Store in session state
...
# --- Main processing block: Operates on st.session_state.data_df ---
if st.session_state.data_df is not None and not st.session_state.data_df.empty:
    ...
    w_series = w_series / w_sum  # Normalize
    weights = w_series.to_dict()  # Replace old weights with normalized values
        ...
        if not actual_factors_in_df:
            ...
            for factor in actual_factors_in_df:
                if 'zeta' in factor.lower() or 'potential' in factor.lower():
                    ...
                    continue
                if factor in df_processed.columns and df_processed[factor].min() < 0: # Check factor exists
            ...
            for factor in maximize_factors_selected:
                if factor in df_processed.columns:
                    if df_processed[factor].max() != 0: # Avoid division by zero or all zeros
                        df_processed[factor + '_normalized'] = df_processed[factor] / df_processed[factor].max()
                    else: # All values are zero or column max is zero
            ...
            for factor in minimize_factors_selected:
                if factor in df_processed.columns:
                    ...
                    if 'zeta' in factor.lower() or 'potential' in factor.lower() or (series_to_normalize < 0).any():
                        ...
                        if (series_to_normalize <= 0).all() and series_to_normalize.min() < 0: # All non-positive, at least one negative
                             df_processed[norm_col_name] = series_to_normalize / series_to_normalize.min()
                        else: # Fallback or other cases (e.g. minimize towards zero, or positive values)
                            # Standard minimize: min_value / value (for positive values)
                            # If values can be zero, replace with small number to avoid division by zero
                            temp_series = series_to_normalize.replace(0, 1e-9) # Use a very small number
                            if temp_series.min() > 0 : # All positive after replacement
                                df_processed[norm_col_name] = temp_series.min() / temp_series
                            elif temp_series.abs().max() > 0 : # If there are non-zero values (absolute)
                                # Normalize based on distance to zero (smaller absolute value is better)
                                df_processed[norm_col_name] = 1 - (temp_series.abs() / temp_series.abs().max())
                            else: # All zeros
                                df_processed[norm_col_name] = 1.0 # All are at the minimum possible, so perfect score
                    elif (series_to_normalize >= 0).all(): # All values are positive or zero
                        min_val = series_to_normalize.min()
                        max_val = series_to_normalize.max()
                        if max_val == min_val : # all values are the same
                            df_processed[norm_col_name] = 1.0 if min_val == 0 else 0.0 # if all zero, score 1, else if all same non-zero, score 0 (as cant improve)
                        elif max_val > 0 :
                             # Normalize so that min_val gets 1, max_val gets 0
                             # (max_val - value) / (max_val - min_val)
                             # Or simpler: min_value / value if min_value > 0
                             if min_val > 0:
                    ...
                    else: # Mixed positive/negative, not explicitly zeta - treat as minimize towards zero
            ...
            if not normalized_factors_present:
                ...
                df_processed['Score'] = 0.0 # Ensure float
                for factor in actual_factors_in_df:
                    norm_col = factor + '_normalized'
                    if norm_col in df_processed.columns and factor in weights:
                        # Ensure normalized scores are not NaN before weighting
                ...
                # --- Visualizations ---
                # (Ensure plots use df_processed and normalized_factors_present)
                # (Keeping visualization code as is, assuming it's mostly correct once data flows)
                ...
                if not df_processed.empty and 'Score' in df_processed.columns and normalized_factors_present:
                    ...
                    if not top_methods_df.empty and not top_methods_df[normalized_factors_present].empty:
                        ...
                        for method_idx, method_name_radar in enumerate(top_methods_df.index):
                            values = radar_data.loc[method_name_radar].fillna(0).tolist() # fillna for safety
                            values += values[:1] 
                ...
                if len(normalized_factors_present) >= 2 and 'Score' in df_processed.columns and 'Rank' in df_processed.columns:
                    ...
                    if all(col in df_processed.columns for col in normalized_factors_present):
                        ...
                        # norm = mcolors.Normalize(vmin=1, vmax=len(df_processed))
                        
                        # cmap = cm.get_cmap('viridis')
                        # df_processed['Color'] = df_processed['Rank'].apply(lambda r: cmap(norm(r)))
                        # cmap = df_processed['Color']
                        ...
                        # Add Rank number inside each point
                        for method_name in df_processed.index:
                            ...
                            # ax_3d.text(x, y, z, str(rank), size=7, color='white', ha='center', va='center', weight='normal')
                        ...
                        # Custom legend with method names
                        import matplotlib.patches as mpatches
                        from matplotlib.legend_handler import HandlerPatch
                        
                        # Custom handler to draw number inside circle
                        ...
                        # Create custom legend elements with circles and rank numbers inside
                        cmap = plt.get_cmap('viridis')
                        ...
                        for method_name in df_processed.index:
                            ...
                            # Create a dummy Circle for the legend handle
                            dummy_circle = mpatches.Circle((0, 0), radius=6)
                            # legend_elements.append((dummy_circle, method_name))
                            legend_elements.append((dummy_circle, method_name, rank))  # include rank as a separate value
                            # Add custom handler for each handle
                            handler_map[dummy_circle] = HandlerCircleWithText(rank=rank, color=color)
                        
                        # Sort legend by rank
                        # legend_elements = sorted(legend_elements, key=lambda x: int(x[1].split(":")[0]))
                        # handles, labels = zip(*legend_elements)
                        legend_elements = sorted(legend_elements, key=lambda x: x[2])  # sort by rank
                ...
                # if len(normalized_factors_present) >= 2 and 'Score' in df_processed.columns and 'Rank' in df_processed.columns:
                #     st.subheader("3D Scatter Plot: Distance to Ideal Solutions & Scores")
                #     # Ensure normalized columns exist before calculating distances
                #     if all(col in df_processed.columns for col in normalized_factors_present):
                #         ideal_best = df_processed[normalized_factors_present].max()
                #         ideal_worst = df_processed[normalized_factors_present].min()
                        
                #         df_processed['Distance to Ideal Best'] = np.sqrt(np.sum((df_processed[normalized_factors_present].fillna(0) - ideal_best.fillna(0)) ** 2, axis=1))
                #         df_processed['Distance to Ideal Worst'] = np.sqrt(np.sum((df_processed[normalized_factors_present].fillna(0) - ideal_worst.fillna(0)) ** 2, axis=1))
                        
                #         fig_3d = plt.figure(figsize=(10, 8))
                #         ax_3d = fig_3d.add_subplot(111, projection='3d')
                #         scatter = ax_3d.scatter(
                #             df_processed['Distance to Ideal Best'], df_processed['Distance to Ideal Worst'], df_processed['Score'], 
                #             c=df_processed['Rank'], cmap='viridis', s=100)
                        
                #         ax_3d.set_xlabel('Distance to Ideal Best (Lower is Better)')
                #         ax_3d.set_ylabel('Distance to Ideal Worst (Higher is Better)')
                #         ax_3d.set_zlabel('Score (Higher is Better)')
                #         cbar = plt.colorbar(scatter, ax=ax_3d, fraction=0.03, pad=0.1)
                #         cbar.set_label('Rank')
                #         for method_name_3d in df_processed.index:
                #             ax_3d.text(
                #                 df_processed.loc[method_name_3d, 'Distance to Ideal Best'], 
                #                 df_processed.loc[method_name_3d, 'Distance to Ideal Worst'], 
                #                 df_processed.loc[method_name_3d, 'Score'] + 0.02 * (df_processed['Score'].max() if df_processed['Score'].max() > 0 else 1),
                #                 method_name_3d, size=8, zorder=1, color='k', ha='center')
                #         plt.title('3D Plot: Distances & Scores')
                #         st.pyplot(fig_3d)

                # --- Report Generation ---
                # (Keeping report generation code as is, assuming it's mostly correct)
                doc = Document()
    ...
    # This covers cases where data_df might be None or empty after an attempt
